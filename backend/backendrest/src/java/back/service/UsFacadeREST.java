...
public class UsFacadeREST extends AbstractFacade<Us> {
    ...
    public void create(Us entity) {
        Query q = em.createNamedQuery("Us.ultimoId", Us.class);
        Integer idus =  (Integer) q.setMaxResults(1).getSingleResult();
        idus = idus + 1;
        Integer idp = (Integer) entity.getUsPK().getIdproyecto();
        Integer ids = (Integer) entity.getUsPK().getIdsprint();
        UsPK iduspk = new UsPK(idus,idp,ids);
        entity.setUsPK(iduspk);
        super.create(entity);
    }
    ...
    public List<Notasus> listaNotas(@PathParam("idus")Integer idus) {
        Query q = em.createNamedQuery("Notasus.findByIdus", Notasus.class);
        q.setParameter("idus", idus);
